...
@app.get("/")
async def root():
    return {"message": "✅ System Board Audit Backend is running with lazy model loading"}
...
async def predict(file: UploadFile = File(...)):
    ...
    def detect(model, image, label_map, key):
        ...
        if selected_class and selected_box:
            # Match full class names exactly
            if selected_class == "connect" or selected_class == "connected":
                component_results[key]["status"] = "Connected"
                component_results[key]["color"] = (0, 255, 0)
            elif selected_class == "disconnect" or selected_class == "disconnected":
